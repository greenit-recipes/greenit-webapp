...
export const SimilarRecipe: React.FC<ISimilarRecipe> = (data) => {
  ...
  const { data: dataSimilarRecipe } = useRecipesQuery({
    ...
    // @ts-ignore
    variables: { first: 5, filter: { category: [data?.data?.category?.name], isRandomList: true, excludeId: data?.data?.id } },
  });
  ...
  return (
    <Carousel
      ...
      containerClass={
        isMobile ? "carousel-similar-recipe-mobile" : "carousel-similar-recipe"
      }
